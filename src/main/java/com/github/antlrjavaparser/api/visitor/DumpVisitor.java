...
public final class DumpVisitor implements VoidVisitor<Object> {
	...
	public void visit(LambdaExpr n, Object arg) {
		printBeginComments(n.getBeginComments(), arg);
		/*
		 * if (n.getScope() != null) { n.getScope().accept(this, arg);
		 * printer.print("."); } printTypeArgs(n.getTypeArgs(), arg);
		 * printer.print(n.getName()); printArguments(n.getArgs(), arg);
		 */
	...
	@Override
	public void visit(FormalLambdaParameterExpr n, Object arg) {
		printBeginComments(n.getBeginComments(), arg);
	...
	@Override
	public void visit(SimpleLambdaParameterExpr n, Object arg) {
		printBeginComments(n.getBeginComments(), arg);

		List<String> identifiers = n.getIdentifiers();
		if (identifiers != null) {
			if (identifiers.size() == 1) {
				printer.print(identifiers.get(0));
			} else {
				printer.print("(");
				for (Iterator<String> i = identifiers.iterator(); i.hasNext();) {
					printer.print(i.next());
	...
	private void printBeginComments(List<Comment> beginComments, Object arg) {
		...
		for (Comment comment : beginComments) {
			printComment(comment, printer.getLevel(),
					CommentFormatter.CommentLocation.BEGINNING);
		}
	}

	private void printInternalComments(List<Comment> internalComments,
			Object arg) {
		...
		for (Comment comment : internalComments) {
			printComment(comment, printer.getLevel(),
					CommentFormatter.CommentLocation.INTERNAL);
		}
	}
